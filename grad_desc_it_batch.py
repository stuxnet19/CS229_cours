	...
	for index1 in range(len(alpha)) :	
		for i in range(nb_iter) :
			...
			for j in range(X.shape[1]) :
				tmp_thetas[index1][j] = thetas[index1][j] - alpha[index1] * 1/m* partial_der_cost_equation(X,y,thetas[index1],j)
			
			for j in range(X.shape[1]) :
				thetas[index1] = tmp_thetas[index1]
...
def partial_der_cost_equation(x,y,theta,j):
	s = 0
